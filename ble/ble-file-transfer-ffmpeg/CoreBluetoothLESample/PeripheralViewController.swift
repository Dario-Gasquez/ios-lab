...
class PeripheralViewController: UIViewController {
    ...
    private func sendData() {
        ...
        // First up, check if we're meant to be sending an EOF
        if PeripheralViewController.sendingEOF {
            // send it
            let didSend = peripheralManager.updateValue("EOF".data(using: .utf8)!, for: transferCharacteristic, onSubscribedCentrals: nil)
            // Did it send?
            if didSend {
                // It did, so mark it as sent
                PeripheralViewController.sendingEOF = false
                os_log("Sent: EOF")
            }
        ...
        while didSend {
            ...
            if sendDataIndex >= dataToSend.count {
                ...
                PeripheralViewController.sendingEOF = true
                
                //Send it
                let eofSent = peripheralManager.updateValue("EOF".data(using: .utf8)!,
    ...
    //    private func dataFromAudioFile(filename: String, fileExtension: String) -> Data? {
...
extension PeripheralViewController: CBPeripheralManagerDelegate {
    ...
    func peripheralManager(_ peripheral: CBPeripheralManager, central: CBCentral, didSubscribeTo characteristic: CBCharacteristic) {
        ...
        let fileToSend = "short-heartbeat"
//        let fileToSend = "gs-16b-1c-44100hz"
        dataToSend = Utilities.dataFromAudioFile(filename: fileToSend, fileExtension: "ogg")!
    ...
    func peripheralManagerIsReady(toUpdateSubscribers peripheral: CBPeripheralManager) {
        // Start sending again
        os_log("peripheralManagerIsReady(toUpdateSubscribers peripheral: %@", peripheral)
